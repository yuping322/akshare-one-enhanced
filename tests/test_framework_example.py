...
def is_string_dtype(dtype) -> bool:
    """Check if a dtype represents a string type (supports pandas 3.0+ StringDtype)."""
    return dtype == "object" or str(dtype) in ("str", "string") or str(dtype).startswith("StringDtype")
...
class TestContractTestingExample:
    ...
    def test_create_and_validate_golden_sample(self, tmp_path):
        ...
        # Save golden sample
        validator.save_golden_sample("example_data", df, metadata={"description": "Example data for testing"})
    ...
    def test_schema_validation_detects_changes(self, tmp_path):
        ...
        df_original = pd.DataFrame({"date": ["2024-01-01"], "symbol": ["600000"], "close": [10.5]})
        ...
        df_extra = pd.DataFrame({"date": ["2024-01-01"], "symbol": ["600000"], "close": [10.5], "extra_column": [100]})
...
class TestDataFrameValidatorExample:
    ...
    def test_validate_required_columns(self):
        ...
        df = pd.DataFrame({"date": ["2024-01-01"], "symbol": ["600000"], "close": [10.5]})
    ...
    def test_validate_no_null_columns(self):
        ...
        df = pd.DataFrame({"date": ["2024-01-01", "2024-01-02"], "symbol": ["600000", "600000"], "close": [10.5, None]})
    ...
    def test_validate_date_range(self):
        ...
        df = pd.DataFrame({"date": ["2024-01-01", "2024-01-15", "2024-01-31"]})
        ...
        df_outside = pd.DataFrame({"date": ["2024-01-01", "2024-02-01"]})
    ...
    def test_validate_numeric_range(self):
        ...
        df_negative = pd.DataFrame({"price": [10.0, -5.0, 30.0]})
    ...
    def test_validate_json_compatible(self):
        ...
        df_valid = pd.DataFrame(
            {"date": ["2024-01-01", "2024-01-02"], "symbol": ["600000", "600000"], "close": [10.5, 11.0]}
        )
...
class TestMockDataGeneratorExample:
    ...
    def test_generate_mock_dataframe(self, mock_data_generator):
        ...
        df = mock_data_generator.generate_mock_dataframe(
            columns=["date", "symbol", "close", "volume"], row_count=10, date_column="date", start_date="2024-01-01"
        )
        ...
        # Check date column
        assert is_string_dtype(df["date"].dtype)

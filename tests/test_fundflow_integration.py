...
class TestCompleteFlowIndustry:
    ...
    def test_complete_industry_flow(self):
        ...
        validator.validate_required_columns(industries, ["sector_code", "sector_name", "constituent_count"])
        ...
        for symbol in constituents["symbol"].head(3):
            ...
            if not fund_flow.empty:
                validator.validate_required_columns(
                    fund_flow, ["date", "symbol", "close_price", "fundflow_main_net_inflow"]
                )

                # Verify symbol matches
                assert all(fund_flow["symbol"] == symbol), f"All rows should have symbol {symbol}"
    ...
    def test_industry_sector_fund_flow(self):
        ...
        validator.validate_required_columns(
            sector_flow, ["date", "sector_code", "sector_name", "sector_type", "main_net_inflow"]
        )
        validator.validate_json_compatible(sector_flow)

        # Verify sector_type is 'industry'
        assert all(sector_flow["sector_type"] == "industry"), "All rows should have sector_type='industry'"
...
class TestCompleteFlowConcept:
    ...
    def test_complete_concept_flow(self):
        ...
        validator.validate_required_columns(concepts, ["sector_code", "sector_name", "constituent_count"])
    ...
    def test_concept_sector_fund_flow(self):
        ...
        assert all(sector_flow["sector_type"] == "concept"), "All rows should have sector_type='concept'"
...
class TestRealDataFetching:
    ...
    def test_stock_fund_flow_real_data_quality(self):
        ...
        for symbol in test_symbols:
            ...
            if not df.empty:
                ...
                validator.validate_required_columns(
                    df, ["date", "symbol", "close_price", "pct_change", "fundflow_main_net_inflow"]
                )
    ...
    def test_main_fund_flow_rank_real_data(self):
        ...
        # Validate structure
        validator.validate_required_columns(df, ["rank", "symbol", "name", "main_net_inflow", "pct_change"])
        validator.validate_json_compatible(df)
    ...
    def test_sector_lists_completeness(self):
        ...
        assert not industries["sector_code"].duplicated().any(), "Industry codes should be unique"
        ...
        # Check for duplicates
        assert not concepts["sector_code"].duplicated().any(), "Concept codes should be unique"
    ...
    def test_fund_flow_data_consistency(self):
        ...
        if not df.empty and len(df) > 0:
            ...
            for _idx, row in df.iterrows():
                if (
                    pd.notna(row["fundflow_main_net_inflow"])
                    and pd.notna(row["fundflow_super_large_net_inflow"])
                    and pd.notna(row["fundflow_large_net_inflow"])
                ):

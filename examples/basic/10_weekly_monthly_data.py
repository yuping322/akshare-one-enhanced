...
def example_compare_intervals():
    ...
    df_day = get_hist_data(
        symbol=symbol, interval="day", start_date=start_date, end_date=end_date, adjust="qfq", source="eastmoney_direct"
    )
    ...
    print(f"\n数据压缩比（以日线为基准）：")
    print(f"  周线/日线: {len(df_week) / len(df_day):.2%}")
    print(f"  月线/日线: {len(df_month) / len(df_day):.2%}")

...
def get_all_securities(types: Optional[List[str]] = None, date: Optional[str] = None) -> pd.DataFrame:
    ...
    try:
        ...
        df["code"] = df["code"].astype(str).str.zfill(SYMBOL_ZFILL_WIDTH)
        df["jq_code"] = df["code"].apply(lambda x: f"{x}.XSHG" if x.startswith("6") else f"{x}.XSHE")
...
def get_security_info(code: str, date: Optional[str] = None):
    ...
    try:
        clean = code.split(".")[0].zfill(SYMBOL_ZFILL_WIDTH)
        df = ak.stock_individual_info_em(symbol=clean)
        if df is None or df.empty:
            return {"code": code, "display_name": code, "name": code, "start_date": "2000-01-01", "type": "stock"}

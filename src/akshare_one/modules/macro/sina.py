...
class SinaMacroProvider(MacroProvider):
    ...
    def get_lpr_rate(
        ...
        return pd.DataFrame(columns=[
            'date', 'loan_1y', 'loan_5y', 'deposit_1y', 'deposit_5y'
        ])
    ...
    def get_pmi_index(
        ...
        self.validate_date_range(start_date, end_date)
        return pd.DataFrame(columns=[
            'date', 'pmi_type', 'pmi', 'previous', 'change'
        ])
    ...
    def get_shibor_rate(
        ...
        self.validate_date_range(start_date, end_date)
        return pd.DataFrame(columns=[
            'date', 'overnight', '1w', '2w', '1m', '3m', '6m', '1y'
        ])
    ...
    def get_social_financing(
        ...
        self.validate_date_range(start_date, end_date)
        return pd.DataFrame(columns=[
            'date', 'total', 'RMB', 'foreign_currency', 'bill', 'bond', 'equity'
        ])

...
class EastmoneyFundFlowProvider(FundFlowProvider):
    ...
    def get_sector_fund_flow(self, sector_type: str, start_date: str, end_date: str) -> pd.DataFrame:
        ...
        try:
            ...
            standardized["leading_stock"] = raw_df.get("今日主力净流入最大股", pd.Series([""] * len(raw_df))).astype(
                str
            )
    ...
    def get_main_fund_flow_rank(self, date: str, indicator: str) -> pd.DataFrame:
        ...
        if indicator not in ["net_inflow", "net_inflow_rate"]:
            raise ValueError(f"Invalid indicator: {indicator}. Must be 'net_inflow' or 'net_inflow_rate'")

        try:
            import akshare as ak

            # Map indicator to akshare parameter
            indicator_map = {"net_inflow": "今日", "net_inflow_rate": "今日"}

            # Call akshare function
            raw_df = ak.stock_individual_fund_flow_rank(indicator=indicator_map[indicator])

            if raw_df.empty:
                return self.create_empty_dataframe(["rank", "symbol", "name", "main_net_inflow", "pct_change"])
    ...
    def get_industry_list(self) -> pd.DataFrame:
        ...
        try:
            ...
            if raw_df.empty:
                return self.create_empty_dataframe(["sector_code", "sector_name", "constituent_count"])

            # Standardize the data
            standardized = pd.DataFrame()
            standardized["sector_code"] = raw_df["板块代码"].astype(str)
            standardized["sector_name"] = raw_df["板块名称"].astype(str)
            standardized["constituent_count"] = pd.to_numeric(
                raw_df.get("公司数量", pd.Series([0] * len(raw_df))), errors="coerce"
            ).astype(int)
    ...
    def get_industry_constituents(self, industry_code: str) -> pd.DataFrame:
        ...
        try:
            ...
            if raw_df.empty:
                return self.create_empty_dataframe(["symbol", "name", "weight"])

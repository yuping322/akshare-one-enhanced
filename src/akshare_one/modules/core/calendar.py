...
def transform_date(
    date_input: str | date | dt | pd.Timestamp, output_type: Literal["date", "datetime", "str", "timestamp"] = "date"
...
def get_trade_dates_between(
    start_date: str | date | dt | pd.Timestamp, end_date: str | date | dt | pd.Timestamp
) -> list[date]:

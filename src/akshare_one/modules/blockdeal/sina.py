"""
Sina block deal data provider.

This module implements the block deal data provider using Sina as the data source.
"""
...
class SinaBlockDealProvider(BlockDealProvider):
    ...
    This provider wraps akshare functions to fetch block deal data from Sina
    ...
    def get_block_deal(self, symbol: str | None, start_date: str, end_date: str) -> pd.DataFrame:
        """
        Get block deal transaction details from Sina.
        ...
        try:
            ...
            raise RuntimeError(f"Failed to fetch block deal data from Sina: {e}") from e

    def get_block_deal_summary(self, start_date: str, end_date: str, group_by: str) -> pd.DataFrame:
        """
        Get block deal summary statistics from Sina.
        ...
        try:
            ...
            if group_by == "stock":
                ...
                summary = (
                    all_data.groupby("date").agg({"amount": ["count", "sum"], "premium_rate": "mean"}).reset_index()
                )
            ...
            raise RuntimeError(f"Failed to fetch block deal summary from Sina: {e}") from e

"""
Factor normalization: Winsorization, Standardization, Neutralization.
"""
...
def winsorize_med(
    ...
    if axis == 0:
        ...
        for idx in df.index:
            df.loc[idx] = _winsorize_med_series(df.loc[idx], scale, inclusive, inf2nan=False)
    return df


def standardlize(
    factor_data: pd.DataFrame | pd.Series, inf2nan: bool = True, axis: int = 0
) -> pd.DataFrame | pd.Series:

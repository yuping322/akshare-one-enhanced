...
class SinaLimitUpDownProvider(LimitUpDownProvider):
    ...
    def get_limit_up_pool(self, date: str) -> pd.DataFrame:
        ...
        return pd.DataFrame(columns=[
            'date', 'symbol', 'name', 'price', 'change_pct',
            'volume', 'turnover_rate', 'first_limit_time', 'reason'
        ])
    ...
    def get_limit_up_stats(
        ...
        return pd.DataFrame(columns=[
            'date', 'limit_up_count', 'limit_down_count', 'continue_limit_up_count',
            'avg_change_pct', 'total_volume'
        ])

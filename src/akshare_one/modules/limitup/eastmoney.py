...
class EastmoneyLimitUpDownProvider(LimitUpDownProvider):
    ...
    def get_limit_down_pool(self, date: str) -> pd.DataFrame:
        ...
        try:
            ...
            if raw_df.empty:
                return self.create_empty_dataframe(
                    ["date", "symbol", "name", "close_price", "limit_down_time", "open_count", "turnover_rate"]
                )
        ...
        try:
            ...
            while current <= end:
                ...
                try:
                    ...
                    results.append(
                        {"date": date_display, "limit_up_count": 0, "limit_down_count": 0, "broken_rate": 0.0}
                    )

                current += timedelta(days=1)

            if not results:
                return self.create_empty_dataframe(["date", "limit_up_count", "limit_down_count", "broken_rate"])

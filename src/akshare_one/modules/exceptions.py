...
def handle_upstream_error(error: Exception, source: str) -> MarketDataError:
    ...
    if isinstance(error, requests.Timeout):
        return DataSourceUnavailableError(
            f"Timeout connecting to {source}: {error}"
        )
    elif isinstance(error, requests.ConnectionError):
        return DataSourceUnavailableError(
            f"Connection error to {source}: {error}"
        )
    elif isinstance(error, requests.HTTPError):
        if error.response.status_code == 429:
            return RateLimitError(
                f"Rate limit exceeded for {source}: {error}"
            )
        elif 400 <= error.response.status_code < 500:
            return InvalidParameterError(
                f"Client error from {source}: {error}"
            )
        else:
            return DataSourceUnavailableError(
                f"Server error from {source}: {error}"
            )
    elif isinstance(error, (KeyError, AttributeError)):
        return UpstreamChangedError(
            f"Unexpected data structure from {source}: {error}"
        )
    else:
        return MarketDataError(
            f"Unexpected error from {source}: {error}"
        )

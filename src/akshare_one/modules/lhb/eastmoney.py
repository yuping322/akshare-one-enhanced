...
class EastmoneyDragonTigerProvider(DragonTigerProvider):
        ...
        try:
            ...
            if raw_df.empty:
                return self.create_empty_dataframe([
                    'date', 'symbol', 'name', 'close_price', 'pct_change', 'reason',
                    'buy_amount', 'sell_amount', 'net_amount', 'total_amount', 'turnover_rate'
                ])
    ...
    def get_dragon_tiger_summary(
        ...
        if group_by not in ['stock', 'broker', 'reason']:
            raise ValueError(
                ...
                "Must be 'stock', 'broker', or 'reason'"
            )
        ...
        try:
            ...
            if group_by == 'stock':
                ...
                if raw_df.empty:
                    return self.create_empty_dataframe([
                        'symbol', 'name', 'list_count', 'net_buy_amount',
                ...
                if raw_df.empty:
                    return self.create_empty_dataframe([
                        'broker_name', 'list_count', 'buy_amount', 'buy_count',
                        'sell_amount', 'sell_count', 'total_amount'
                    ])
                ...
                if raw_df.empty:
                    return self.create_empty_dataframe([
                        'reason', 'list_count', 'net_buy_amount',
                        'buy_amount', 'sell_amount', 'total_amount'
                    ])
                
                # Group by reason
                grouped = raw_df.groupby('上榜原因').agg({
                    '代码': 'count',
                    '龙虎榜净买额': 'sum',
                    '龙虎榜买入额': 'sum',
                    '龙虎榜卖出额': 'sum',
    ...
    def get_dragon_tiger_broker_stats(
        ...
        try:
            ...
            if raw_df.empty:
                return self.create_empty_dataframe([
                    'rank', 'broker_name', 'list_count', 'buy_amount', 'buy_count',
                    'sell_amount', 'sell_count', 'net_amount', 'total_amount'
                ])

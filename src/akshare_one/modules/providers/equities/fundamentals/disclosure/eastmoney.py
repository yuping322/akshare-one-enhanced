...
@DisclosureFactory.register("eastmoney")
class EastmoneyDisclosureProvider(DisclosureProvider):
    ...
    def _standardize_disclosure_news(
        self, raw_df: pd.DataFrame, symbol_filter: str | None, category_filter: str
    ) -> pd.DataFrame:
        """
    ...
    def _standardize_dividend_data(
        self, raw_df: pd.DataFrame, symbol: str, start_date: str, end_date: str
    ) -> pd.DataFrame:
        """
        ...
        standardized["record_date"] = pd.to_datetime(raw_df["股权登记日"], errors="coerce").dt.strftime("%Y-%m-%d")

        standardized["ex_dividend_date"] = pd.to_datetime(raw_df["除权日"], errors="coerce").dt.strftime("%Y-%m-%d")

        standardized["payment_date"] = pd.to_datetime(raw_df["派息日"], errors="coerce").dt.strftime("%Y-%m-%d")
        ...
        standardized = standardized.sort_values("ex_dividend_date", ascending=False, na_position="last").reset_index(
            drop=True
        )
    ...
    def _standardize_repurchase_data(
        self, raw_df: pd.DataFrame, symbol_filter: str | None, start_date: str, end_date: str
    ) -> pd.DataFrame:
        """
        ...
        standardized["amount"] = raw_df["已回购金额"].fillna(raw_df["计划回购金额区间-下限"])
    ...
    def _standardize_delist_data(self, raw_df: pd.DataFrame, market: str) -> pd.DataFrame:
        """
        ...
        standardized["st_type"] = standardized["name"].apply(lambda x: self.standardize_st_type(x))

        # Determine risk level from name (not ST type, as name contains more info)
        standardized["risk_level"] = standardized["name"].apply(lambda x: self.standardize_risk_level(x))

        # Convert announcement date
        standardized["announcement_date"] = pd.to_datetime(raw_df[date_col], errors="coerce").dt.strftime("%Y-%m-%d")

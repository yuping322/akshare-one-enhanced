        ...
        for new_field in df.columns:
            ...
            if new_field in self.reverse_aliases:
                for old_field in self.reverse_aliases[new_field]:
                    ...
                    added_count += 1
                    logger.debug(
                        f"Added alias: {old_field} -> {new_field}"
                    )
        
        if added_count > 0:
            logger.info(
                f"Added {added_count} alias fields to DataFrame"
            )
    ...
    def resolve_field_access(
        ...
        if field_name in self.aliases:
            ...
            if standard_name in df.columns:
                logger.debug(
                    f"Resolved deprecated field access: {field_name} -> {standard_name}"
                )

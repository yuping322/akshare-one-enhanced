"""
Factory for creating equity pledge data providers.

This module implements the factory pattern for creating equity pledge data providers
based on the specified data source.
"""

from typing import Dict, Type

from .base import EquityPledgeProvider
from .eastmoney import EastmoneyEquityPledgeProvider
from .sina import SinaEquityPledgeProvider


class EquityPledgeFactory:
    """
    Factory class for creating equity pledge data providers.
    
    Supports multiple data sources and provides a unified interface
    for creating provider instances.
    """
    
    # Registry of available providers
    _providers: Dict[str, Type[EquityPledgeProvider]] = {
        'eastmoney': EastmoneyEquityPledgeProvider,
        'sina': SinaEquityPledgeProvider,
    }
    
    @classmethod
    def get_provider(cls, source: str, **kwargs) -> EquityPledgeProvider:
        """
        Create an equity pledge data provider instance.
        
        Args:
            source: Data source name ('eastmoney')
            **kwargs: Additional parameters for the provider
        
        Returns:
            EquityPledgeProvider: Provider instance
        
        Raises:
            ValueError: If the specified source is not supported
        
        Example:
            >>> provider = EquityPledgeFactory.get_provider('eastmoney')
            >>> df = provider.get_equity_pledge('600000', '2024-01-01', '2024-01-31')
        """
        if source not in cls._providers:
            available = ', '.join(cls._providers.keys())
            raise ValueError(
                f"Unsupported data source: {source}. "
                f"Available sources: {available}"
            )
        
        provider_class = cls._providers[source]
        return provider_class(**kwargs)
    
    @classmethod
    def register_provider(cls, source: str, provider_class: Type[EquityPledgeProvider]) -> None:
        """
        Register a new equity pledge data provider.
        
        This allows extending the factory with custom providers.
        
        Args:
            source: Data source name
            provider_class: Provider class (must inherit from EquityPledgeProvider)
        
        Raises:
            TypeError: If provider_class is not a subclass of EquityPledgeProvider
        
        Example:
            >>> class CustomProvider(EquityPledgeProvider):
            ...     pass
            >>> EquityPledgeFactory.register_provider('custom', CustomProvider)
        """
        if not issubclass(provider_class, EquityPledgeProvider):
            raise TypeError(
                f"Provider class must inherit from EquityPledgeProvider, "
                f"got {provider_class.__name__}"
            )
        
        cls._providers[source] = provider_class
    
    @classmethod
    def list_sources(cls) -> list:
        """
        List all available data sources.
        
        Returns:
            list: List of available source names
        
        Example:
            >>> sources = EquityPledgeFactory.list_sources()
            >>> print(sources)
            ['eastmoney']
        """
        return list(cls._providers.keys())
